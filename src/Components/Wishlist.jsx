...
const Wishlist = () => {
  ...
  if (loadingWishlist) {
    return (
      <div className="text-center py-20 text-gray-600">Loading wishlist...</div>
    );
  }
                ...
                >
                  <Link to={`/product/${product._id}`}>
                    <img
                      ...
                      alt={product.name}
                      className="h-40 md:h-60 w-full object-cover rounded-lg mb-3"
                    />
                  </Link>
                  ...
                  {/* Add to Cart */}
                  <button
                    onClick={() => !itemInCart && addToCart(product._id)}
                    disabled={!!itemInCart}
                  ...
                  >
                    {itemInCart ? "Item in Cart" : "Add to Cart"}
                  </button>
                </div>
              );
            })}
